...
- Hall Call 在新乘客、到层、上下客完成/状态变化等模型事件后重评估，不在纯帧边界评估。同一仿真时刻只改派一轮；ETA 至少改善 5 秒才改派，原梯距离请求层 <=1 层（含已到站/正在服务）禁止改派，改派后冷却 10 仿真秒。阈值统一在 Dispatcher.h；Simulation 同步撤销旧梯外呼、添加新梯外呼及更新归属。
- Elevator::RemoveHallCall 只撤销未到当前楼层的指定方向外呼，不能删除内呼或改变当前动作、方向、剩余时间。改派提交只对已选定两台梯准备副本后移动回写，不用于搜索试组合；既有 Elevator 状态机不得重构。
- 联合分配只取最老 3 个未分配外呼，每层搜索最多 3 个候选梯 + 暂不分配，最多 64 个叶组合。只能在局部快照中逐项加任务，最终重算各请求 ETA/载荷。先最大化可分配数量，再按总 Cost、最大 ETA、总 ETA、请求顺序中的电梯 ID 稳定比较；方向惩罚沿用各请求插入前上下文，Aging 保留。允许同梯多请求和部分未分配。
