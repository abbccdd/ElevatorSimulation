...
int main()
{
    ...
    tests.Run("reset creates per-elevator slots", [&] {
        Statistics statistics; statistics.Reset(3);
    ...
    tests.Run("reset zero elevators allowed", [&] {
        Statistics statistics; statistics.Reset(0);
    ...
    tests.Run("boarded moves waiting to riding with T-inclusive mean", [&] {
        Statistics statistics; statistics.Reset(1);
        statistics.PassengerCreated();
        statistics.PassengerBoarded(5.0);
        const auto snapshot = statistics.GetSnapshot();
    ...
    tests.Run("boarded updates running mean and max", [&] {
        Statistics statistics; statistics.Reset(1);
        statistics.PassengerCreated(); statistics.PassengerBoarded(2.0);
        statistics.PassengerCreated(); statistics.PassengerBoarded(8.0);
        const auto snapshot = statistics.GetSnapshot();
    ...
    tests.Run("invalid boarded events rejected", [&] {
        Statistics statistics; statistics.Reset(1);
        bool rejected = false;
        try { statistics.PassengerBoarded(1.0); }
        ...
        {
            bool invalid = false;
            try { statistics.PassengerBoarded(value); }
    ...
    tests.Run("arrived moves riding to arrived with per-elevator count", [&] {
        Statistics statistics; statistics.Reset(2);
        statistics.PassengerCreated(); statistics.PassengerBoarded(3.0);
    ...
    tests.Run("invalid arrived events rejected", [&] {
        ...
        statistics.PassengerCreated(); statistics.PassengerBoarded(1.0);
        for (double value : { -1.0, std::numeric_limits<double>::quiet_NaN(),
            std::numeric_limits<double>::infinity() })
        {
            bool invalid = false;
    ...
    tests.Run("invariants total equals waiting plus riding plus arrived", [&] {
        Statistics statistics; statistics.Reset(2);
        for (int i = 0; i < 3; ++i) statistics.PassengerCreated();
        statistics.PassengerBoarded(1.0);
        statistics.PassengerBoarded(2.0);
    ...
    tests.Run("format summary contains key fields", [&] {
        Statistics statistics; statistics.Reset(2);
        statistics.PassengerCreated(); statistics.PassengerBoarded(4.0);
